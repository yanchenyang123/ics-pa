...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  //I_type
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(dest)=((word_t)src1<(word_t)imm)? 1:0);  //seqz
  ...
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(dest)=(Mr(src1+imm ,4))&255); 
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, SEXT(Mr(src1+imm ,2),16));
  ...
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, if((word_t)src1<(word_t)src2){s->dnpc+=imm-4;});
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, if((word_t)src1>=(word_t)src2){s->dnpc+=imm-4;});
